...
def tab_single(obs, clf):
    st.caption("Enter one water reading. If it looks abnormal, the agent fires a thermal interrogation, "
               "shows the cooling curve, and the AI evaluates the k-value to decide. A single typed "
               "reading has no real sonde, so the pulse is simulated assuming healthy hardware \u2014 to "
               "watch the agent catch a fouled sensor, use 'Run a data file' with logged thermal data.")
    ...
    if st.button("Run reading"):
        ...
        if not (level_off or viol):
            ...
            st.info("Reading is within the tank's learned normal and the SAMAQ limits \u2014 "
                    "nothing to interrogate.")
        else:
            # suspicious -> fire the pulse (simulated healthy probe) and let the SVM read the k-value
            ev = evaluate(row, 0.85, clf)
